									...
									<c:forEach items="${lists }" var="row" varStatus="loop">
										<tr>
											<td class="text-center">
												<!-- 가상번호 --> ${map.totalCount - (((map.nowPage - 1) * map.pageSize) +
                        loop.index) }
											</td>

											<td class="text-left">
												<!-- 제목 --> <a
												href="../DataRoom/DataView?idx=${row.idx }&nowPage=${param.nowPage }&searchColumn=${param.searchColumn}&searchWord=${param.searchWord}">
													${row.title } </a>
											</td>
											...
											<td class="text-center">
												<!-- 첨부파일 --> <c:if test="${not empty row.attachedfile }">
													<a

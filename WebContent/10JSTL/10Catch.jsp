...
<body>
	...
	-에러 내용을 원하는 위치에서 출력할 때 사용한다.
	-JSTL문법 오류는 catch되지 않는다. EL에서만 사용가능하다.
	 -->
	<h2>catch 태그</h2>
	<c:set var="fnum" value="100"/>
	<c:set var="snum" value="0"/>
	
	<h3>에러가 안나는 겨웅 :에러내용이 저장안됨</h3>
	<!-- 
	아래의 경우는 0으로 나눠서 계산식 자체에는 문제가 있지만
	에러가 발생하지는 않기 때문에 변수에 에러내용이 저장되지
	...
	이 경우 500에러가 발생되고, var 속성이 지정한 변수에
	에러내용이 저장된다.
	 -->
	\%{"백" + 100} <- 실행시 500에러 발생됨.
